...
Success responses return standard JSON with a 200 status code.
Error responses return a JSON object containing an `error` message and an appropriate HTTP status code (400, 401, 404, 500).
...
- **503 Service Unavailable**: If the server cache is still initializing.

**Response Body Structure:**
The data is optimized for size.
...
- **File**: `Content-Type: application/x-wireguard-config` with `Content-Disposition` attachment.
