...
@pytest.mark.asyncio
async def test_get_key_rejects_unauthorized_and_invalid_payloads(key_factory) -> None:
    responses = [
    ...
    async with serve(handler) as root:
        async with NordClient(endpoints=endpoints(root)) as client:
            ...
            with pytest.raises(NordClientError, match="not a JSON object"):
                await client.get_key("a" * 64)
            with pytest.raises(NordClientError, match="did not contain"):
                await client.get_key("a" * 64)
            with pytest.raises(NordClientError, match="invalid private key"):
                await client.get_key("a" * 64)
...
async def test_get_geo_accepts_valid_and_rejects_invalid_coordinates() -> None:
    ...
    async with serve(handler) as root:
        async with NordClient(endpoints=endpoints(root)) as client:
            ...
            for _ in range(5):
                with pytest.raises(NordClientError, match=r"invalid coordinates|JSON object"):
                    await client.get_geo()


@pytest.mark.asyncio
async def test_get_servers_accepts_nonempty_array_and_rejects_other_payloads() -> None:
    payloads: list[object] = [[{"hostname": "us1.example.com"}], [], {}]
    ...
    async with serve(handler) as root:
        async with NordClient(endpoints=endpoints(root)) as client:
            ...
            with pytest.raises(NordClientError, match="not a JSON array"):
                await client.get_servers()
    ...
    async with serve(handler) as root:
        async with NordClient(endpoints=endpoints(root)) as client:
            for limit, pattern in [
                ...
                with pytest.raises(NordClientError, match=pattern):
                    await client._get_json(f"{root}/value", None, limit)
...
async def test_get_json_requires_context_and_wraps_transport_failure() -> None:
    ...
    with pytest.raises(RuntimeError, match="async context manager"):
        await client._get_json("https://example.com", None, 64)

    timeout = aiohttp.ClientTimeout(total=0.01)
    async with aiohttp.ClientSession(timeout=timeout) as session:
        client = NordClient(session=session)
        with pytest.raises(NordClientError, match="perform request"):
            await client._get_json("http://127.0.0.1:1", None, 64)
...
async def test_get_json_honors_cancellation() -> None:
    ...
    async with serve(handler) as root:
        async with NordClient(endpoints=endpoints(root)) as client:
            task = asyncio.create_task(client._get_json(f"{root}/slow", None, 64))
            await started.wait()
            task.cancel()
