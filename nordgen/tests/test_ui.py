...
def test_terminal_status_and_progress_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ConsoleManager(
        StringIO(),
...
def test_terminal_progress_records_render_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ConsoleManager(
        StringIO(),
    ...
    FakeProgress.stop_error = OSError("stop")
    try:
        with pytest.raises(ConsoleOutputError):
            with manager.progress(
                2,
        ...
        FakeProgress.stop_error = None

    assert manager.output_error is not None


def test_terminal_status_start_and_stop_failures(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ConsoleManager(
        StringIO(),
    ...
    stopping = FakeStatus(
        stop_error=OSError("stop"),
    )

    monkeypatch.setattr(
        manager.console,
        "status",
        lambda *args, **kwargs: stopping,
    )
...
def test_clear_and_wait_terminal_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    output = StringIO()
    manager = ConsoleManager(
