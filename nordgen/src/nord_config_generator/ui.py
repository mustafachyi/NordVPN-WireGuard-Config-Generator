...
from .models import MAX_KEEPALIVE, GenerationStats, UserPreferences
...
class ProgressHandle:
    def __init__(
        ...
        task_id: TaskID | None,
        record_error: Callable[[BaseException], None] | None = None,
    ) -> None:
...
class ConsoleManager:
    ...
    @property
    def output_error(self) -> BaseException | None:
    ...
    def _emit(self, renderable: object = "", *, end: str = "\n") -> None:
        if self._output_error is not None:
            raise ConsoleOutputError("console output is unavailable") from self._output_error
        try:
    ...
    def header(self) -> None:
        self._emit(
            Panel(
                Text("NordVPN Configuration Generator", style="title"),
                expand=False,
    ...
    def prompt_preferences(
        ...
        if "dns" not in provided:
            announce()
            dns = self._prompt_address("DNS IP", defaults.dns)

        use_ip = defaults.use_ip
        if "use_ip" not in provided:
            announce()
            use_ip = self._prompt_bool("Use IP for endpoints?", defaults.use_ip)
    ...
    def _prompt_string(self, message: str, default: str) -> str:
        self._emit(Text(f"{message} [{default}]: ", style="title"), end="")
        try:
            value = self._read_line(4096)
        except (EOFError, InputTooLongError):
            return default
        normalized = value.strip()
        return normalized or default

    def _prompt_address(self, message: str, default: str) -> str:
        while True:
            value = self._prompt_string(message, default)
            try:
    ...
    def _prompt_bool(self, message: str, default: bool) -> bool:
        ...
        while True:
            self._emit(Text(f"{message} [{prompt_default}]: ", style="title"), end="")
            try:
    ...
    def _prompt_int(self, message: str, default: int, minimum: int, maximum: int) -> int:
        while True:
            value = self._prompt_string(message, str(default))
            try:
                parsed = int(value)
            except ValueError:
                parsed = minimum - 1
    ...
    def _read_line(self, limit: int) -> str:
        characters: list[str] = []
        while True:
    ...
    def status(self, message: str) -> Iterator[None]:
        if not self.output_terminal:
            ...
            yield
            return
        status = self.console.status(Text(message, style="info"), spinner="dots")
        try:
            status.start()
        except (OSError, ValueError) as error:
            self._record_output_error(error)
    ...
    @contextmanager
    def progress(self, total: int, message: str) -> Iterator[ProgressHandle]:
        if not self.output_terminal or total <= 0:
            yield ProgressHandle(None, None)
            return
        progress = Progress(
            SpinnerColumn(),
        ...
        try:
            ...
            raise ConsoleOutputError("write console output") from error
        try:
            yield ProgressHandle(progress, task_id, self._record_output_error)
        finally:
            try:
                progress.stop()
            except (OSError, ValueError) as error:
    ...
    def success(self, message: str) -> None:
        self._emit(Text(f"✓ {message}", style="success"))

    def fail(self, message: str) -> None:
        self._emit(Text(f"✗ {message}", style="error"))

    def info(self, message: str) -> None:
        self._emit(Text(f"→ {message}", style="info"))

    def show_key(self, key: str) -> None:
        self._emit(
            Panel(
                Text(key, style="success"),
    ...
    def summary(self, output_path: str, stats: GenerationStats, duration: float) -> None:
        ...
        self._emit(Panel(grid, title="Complete", border_style="bright_green", expand=False))

    def wait(self) -> None:
        if not self.input_terminal or self._output_error is not None:
            return
        self._emit(Text("Press Enter to exit... ", style="muted"), end="")
        try:
