...
from .wireguard import WireGuardValueError, validate_endpoint, validate_key
...
def parse_servers(
    ...
    for raw_value in raw_servers:
        ...
        for group_value in group_values:
            ...
            if not isinstance(identifier, str) or identifier not in TYPE_GROUPS:
                continue
            group_ids.add(identifier)
            if identifier == "legacy_dedicated_ip":
                has_dedicated = True
        if (
            not group_ids
        ...
        if not isinstance(country_name, str) or not isinstance(city_name, str):
            continue
...
def _find_public_key(technologies: Sequence[object]) -> str:
