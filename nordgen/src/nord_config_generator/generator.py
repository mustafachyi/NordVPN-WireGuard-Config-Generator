...
class Generator:
    ...
    async def process(
        ...
        if observer is None or not valid_coordinates(
            observer.latitude,
            observer.longitude,
        ):
            observer = None
    ...
    def _build_jobs(
        ...
        for server in servers:
            country = canonical_path_segment(
                server.country,
                DIRECTORY_MAX_BYTES,
            )
            city = canonical_path_segment(
                server.city,
                DIRECTORY_MAX_BYTES,
            )
            combo = canonical_path_segment(
                server.combo,
                DIRECTORY_MAX_BYTES,
            )
    ...
    async def _write_jobs(
        ...
        with self.ui.status("Preparing file system..."):
            ...
            for job in jobs:
                try:
                    relative_directory = job.path.parent.relative_to(root)
                except ValueError as error:
                    raise GenerationError(
                        f"configuration path escapes output root: {job.path}"
                    ) from error
                ...
                for segment in relative_directory.parts:
                    if segment == os.pardir:
                        raise GenerationError(f"configuration path escapes output root: {job.path}")
            ...
            try:
                ...
                raise GenerationError(f"prepare output directories: {error}") from error
...
def _write_jobs_parallel(
    ...
    if first_error is not None:
        raise GenerationError(f"write configuration files: {first_error}") from first_error
